...
public class CryptoUtils {
	 ...
	 */
	/*
	 * public static String encryptKeyData(String keyData, Long timestamp,
	 * String dataSecret) {
	 * 
	 * DateFormat df = new SimpleDateFormat(Constants.DATE_TIME_FORMAT);
	 * df.setTimeZone(TimeZone.getTimeZone(Constants.DATE_TIMEZONE)); keyData =
	 * df.format(new Date(timestamp)) + keyData; return
	 * desEncryptToBase64(keyData, dataSecret.getBytes()); }
	 */
	 ...
	 * @return String 加密数据
	 */
	public static String encryptKeyData(String keyData, String timestamp,
			String dataSecret) {
	...
	public static String desEncryptToHex(String input, byte[] keyBytes) {
		...
		try {
			encryptResult = des(input.getBytes(DEFAULT_ENCODING), keyBytes,
					Cipher.ENCRYPT_MODE);
	...
	/**
	 * 使用DES加密或解密无编码的原始字节数组, 返回无编码的字节数组结果. 通用的java .net php
	 * 
	 * @param inputBytes
	 *            原始字节数组
	 * @param keyBytes
	...
	private static IllegalStateException convertRuntimeException(
	 ...
	 * @throws ApiException
	 */
	public static String getKeyData(String encryptKeyData, String dataSecret,
			String timestamp) {
