package com.zeekie.stock.service.sensitive;
...
public class SensitiveWordInit {
	...
	/**
	 * 从数据库读取敏感词库中的内容，将内容添加到set集合中
	 * 
	 * @return
	 * @version 1.0
	 * @throws Exception
	 */
	private Set<String> readSensitiveWordFromDB() throws Exception {
		Set<String> set = null;
		try {
			List<String> sensitiveWord = mapper.querySensitiveWords();
			if (!sensitiveWord.isEmpty()) {
				set = new HashSet<String>();
				for (int i = 0; i <= sensitiveWord.size() - 1; i++) {
					set.add(sensitiveWord.get(i));
				}
			}
		} catch (Exception e) {
			throw e;
